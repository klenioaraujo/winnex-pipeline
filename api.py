...
class WinnexPipeline:
    ...
    def build(self, vectors):
        ...
        if self.dim != self.cfg['dimensions']['input_dim']:
            self.cfg['dimensions']['input_dim'] = self.dim
            # Also update stage dims
            sd = self.cfg['dimensions']['stage_dims']
            self.cfg['dimensions']['stage_dims'] = [d for d in sd if d <= self.dim]
            if len(self.cfg['dimensions']['stage_dims']) < 2:
                # Fallback: use half and full
                self.cfg['dimensions']['stage_dims'] = [
                    max(4, self.dim // 4),
                    max(8, self.dim // 2),
                ]
